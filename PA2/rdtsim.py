...
class EntityA:
    ...
    def output(self, message):
        if self.buffer.qsize() > 0 or self.currentmessage is not None:            # If we are currently waiting for a previous message to be ACKed
            self.buffer.put(message)
            if TRACE > 0:
                print('[EntityA][%s] Waiting for earlier ACK. Added message to buffer. %s\r\n'
                      % (get_time(self), str(message)))
        else:                               # If there are no messages waiting to be ACKed
            self.buffer.put(message)
            if TRACE > 0:
                print('[EntityA][%s] Added message to buffer and Sending message. %s\r\n'
                      % (get_time(self), str(message)))
            self.currentmessage = self.buffer.get()
            self.send_packet()
    ...
    def input(self, packet):
        ...
        if packet.acknum != self.expected_seq():
            if TRACE > 0:
                print(
                    '[EntityA][%s] Received the ACK for the previous packet. Expected: %d. Received: %d.\r\n'
                    % (get_time(self), self.expected_seq(), packet.acknum))
            #self.send_packet()
            return
        # If the packet is valid and the expected packet
        stop_timer(self)
        if TRACE > 0:
            print('[EntityA][%s] Received ACKnum %d for message %s\r\n'
                  % (get_time(self), packet.acknum, str(self.currentmessage)))
        self.increment_seq()
        self.currentmessage = None          # Remove the message from the buffer since its seq was ACKed
        if self.buffer.qsize() > 0:            # If there are more buffered messages waiting to be sent
            self.currentmessage = self.buffer.get()
            if TRACE > 0:
                print('[EntityA][%s] Sending next message in buffer %s...\r\n'
                      % (get_time(self), str(self.currentmessage)))
            self.send_packet()
            start_timer(self, self.timerlen)

    # Sends the oldest object in the buffer to EntityB
    def send_packet(self):
        if self.buffer.qsize() < 1 and self.currentmessage is None:
            if TRACE > 0:
                print('[EntityA][%s] You cannot send a packet when the buffer is empty.\r\n'
                      % (get_time(self)))
            return
        checksum = self.expected_seq() + self.last_received_seq() + sum(self.currentmessage.data)
        packet = Pkt(self.expected_seq(), self.last_received_seq(), checksum, self.currentmessage.data)
    ...
    def expected_seq(self):
        return (self.previousseq + 1) % 2

    def increment_seq(self):
        self.previousseq = (self.previousseq + 1) % 2
...
class EntityB:
    ...
    def input(self, packet):
        ...
        to_layer5(self, msg)
        if TRACE > 0:
            print('[EntityB][%s] Delivered message to layer 5. %s\r\n'
                  % (get_time(self), str(msg)))
        self.increment_seq()
        self.send_ack(packet.payload)
