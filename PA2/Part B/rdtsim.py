...
# Verifies if the packet is corrupted.
#
# Return true if it is corrupted, otherwise false.
def is_corrupted(packet):
    if not isinstance(packet.seqnum, int) or not isinstance(packet.acknum, int) or not isinstance(packet.checksum, int):
        return True

    if packet.payload is None or len(packet.payload) != 20:
        return True

    realchecksum = packet.seqnum + packet.acknum + sum(packet.payload)
    if realchecksum != packet.checksum:
        return True
...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.window_size = math.floor(seqnum_limit / 2)
        self.previousseq = seqnum_limit - 1
        self.buffer = queue.Queue()            # Messages are added when they are received. They are deleted after being ACKed.
        self.window = queue.Queue()            # The messages that fit in the window
        self.timerlen = 39
        self.istimeractive = False
        if TRACE > 0:
            print('[EntityA] seqnum_limit: %d, window_size: %d.\r\n'
                  % (self.seqnum_limit, self.window_size))
    ...
    def output(self, message):
        ...
        if TRACE > 0:
            print('[EntityA][%s] Added message to buffer. %s\r\n'
                  % (get_time(self), str(message)))
        # Check if the packet is capable of being sent
        self.check_send_packet()
    ...
    def input(self, packet):
        # Check for corrupted packets, if corrupted, do nothing
        if is_corrupted(packet):
            if TRACE > 0:
                print('[EntityA][%s] Received a corrupted/invalid packet.\r\n'
                      % (get_time(self)))
            return

        # Check for the correct sequence
        if not self.is_expected_ack(packet.acknum):
            if TRACE > 0:
                print(
                    '[EntityA][%s] Received the ACK for a previous packet. Expected: %d. Received: %d.\r\n'
                    % (get_time(self), self.expected_seq(), packet.acknum))
            #self.send_packet()
            return
        # If the packet is valid and the expected seq
        # Remove all the ACKed packets from window
        for i in range(self.window.qsize()):
            self.window.get()
            if packet.acknum == (self.expected_seq() + i) % self.seqnum_limit:
                break

        if self.istimeractive:
            stop_timer(self)
            self.istimeractive = False
        if self.window.qsize() > 0:
            start_timer(self, self.timerlen)
            self.istimeractive = True
            if TRACE > 0:
                print('[EntityA][%s] The window has moved. Reset timer for %d.\r\n'
                    % (get_time(self), self.timerlen))
        if TRACE > 0:
            print('[EntityA][%s] Received ACKnum %d for message %s\r\n'
                  % (get_time(self), packet.acknum, str(packet.payload)))
        self.previousseq = packet.acknum
        # If there are more buffered messages waiting to be sent
        self.check_send_packet()

    # Sends the oldest object in the buffer to EntityB
    def check_send_packet(self):
        if self.buffer.qsize() < 1:
            if TRACE > 0:
                print('[EntityA][%s] No more packets are currently waiting to be sent.\r\n'
                      % (get_time(self)))
            return
        if self.window.qsize() >= self.window_size:
            if TRACE > 0:
                print('[EntityA][%s] Window is full. Cannot add new packet from buffer.\r\n'
                      % (get_time(self)))
            return

        # There is a packet waiting to be sent and room in the window
        for i in range(min(self.window_size - self.window.qsize(), self.buffer.qsize())):
            currentmessage = self.buffer.get()
            checksum = self.get_next_window_seq() + self.get_prev_window_seq() + sum(currentmessage.data)
            packet = Pkt(self.get_next_window_seq(), self.get_prev_window_seq(), checksum, currentmessage.data)
    ...
    def send_packet(self, packet):
        ...
        if TRACE > 0:
            print('[EntityA][%s] Sent message: %s\r\n'
                  % (get_time(self), str(packet)))
    ...
    def timer_interrupt(self):
        if TRACE > 0:
            print('[EntityA][%s] Timed out!\r\n'
                  % (get_time(self)))
        # Resend all the packets in the window
        for i in range(self.window.qsize()):
            self.send_packet(self.window.get())
        start_timer(self, self.timerlen)
        self.istimeractive = True

    def last_received_seq(self):
        return self.previousseq

    def expected_seq(self):
        return (self.previousseq + 1) % self.seqnum_limit

    def is_expected_ack(self, ack):
        for i in range(self.window.qsize()):
            if ack == (self.expected_seq() + i) % self.seqnum_limit:
                return True
        return False

    # Should be called before adding the packet to the window
    def get_next_window_seq(self):
        return (self.expected_seq() + self.window_size - (self.window_size - self.window.qsize())) % self.seqnum_limit

    # Should be called before adding the packet to the window
    def get_prev_window_seq(self):
        return (self.get_next_window_seq() - 1) % self.seqnum_limit
...
class EntityB:
    ...
    def input(self, packet):
        if TRACE > 0:
            print('[EntityB][%s] Received something... Hopefully its good..\r\n'
                  % (get_time(self)))
        # Check for corrupted packets
        if is_corrupted(packet):
            if TRACE > 0:
                print('[EntityB][%s] Received a corrupted/invalid packet. Resending ACK...\r\n'
                      % (get_time(self)))
            self.send_ack(packet.payload)
            return
        # Check for the correct sequence
        if packet.seqnum != self.expected_seq():
            if TRACE > 0:
                print('[EntityB][%s] Received the wrong sequence of packet. Expected: %d. Received: %d. Resending ACK...\r\n'
                      % (get_time(self), self.expected_seq(), packet.seqnum))
            self.send_ack(packet.payload)
            return
        # If the packet is valid and the expected seq
        ...
        to_layer5(self, msg)
        if TRACE > 0:
            print('[EntityB][%s] Delivered message with ACK %d to layer 5. %s\r\n'
                  % (get_time(self), self.expected_seq(), str(msg)))
        self.increment_seq()
        self.send_ack(packet.payload)
    ...
    # Sends a request to EntityA with the previous ACK
    #
    # payload
    #   type: bytes[Msg.MSG_SIZE]
    def send_ack(self, payload):
        checksum = self.expected_seq() + self.last_received_seq() + sum(payload)
        packet = Pkt(self.expected_seq(), self.last_received_seq(), checksum, payload)
        to_layer3(self, packet)
        if TRACE > 0:
            print('[EntityB][%s] Sent ACK to layer 3: %s\r\n'
                  % (get_time(self), str(packet)))
    ...
    def increment_seq(self):
        self.previousseq = (self.previousseq + 1) % self.seqnum_limit
